...
<script type="text/javascript">
	...
	var referencePointsArray = new Array();
	var referencePointsMap = {};
	var referencePointID = -1; //For selection to remain in the filter

	// Create the list of all available points.
	<c:forEach items="${userPoints}" var="dp"><c:if test="${fn:contains(dp.name, '[REF]') and dp.pointLocator.dataTypeId == 3}">
		referencePointsMap["${dp.id}"] = { id: ${dp.id}, name: '${sst:quotEncode(dp.extendedName)}', type: '<m2m2:translate message="${dp.dataTypeMessage}"/>', fancyName: '${sst:quotEncode(dp.extendedName)}' };
		referencePointsArray.push(referencePointsMap["${dp.id}"]);
	</c:if></c:forEach>
	dojo.require("dojo.store.Memory");
	dojo.require("dijit.form.ComboBox");
	// Create the lookup
	var referencePointLookupBox = new dijit.form.ComboBox({
          store: new dojo.store.Memory({ data: referencePointsArray }),
          labelAttr: "fancyName",
          labelType: "html",
          searchAttr: "name",
          autoComplete: false,
          ...
          onChange: function(point) {
              if (this.item) {
            	  referencePointID = this.item.id;
              }
          }
      }, "referencePointLookup");
	/**
	...
	function setLoggingProperties(vo) {
		...
		if (vo.loggingType == <%= DataPointVO.LoggingTypes.CUSTOM_REFERENCE_POINT %>) {
			referencePointID = vo.referencePointID;
			var referencePoint = referencePointsMap[referencePointID];
			if (referencePoint) {
				dojo.byId("referencePointLookup").value = referencePoint.name;
			} else {
				dojo.byId("referencePointLookup").value = referencePointID;
			}
		}
	...
	function getLoggingProperties(vo) {
		...
		if(vo.loggingType == <%= DataPointVO.LoggingTypes.CUSTOM_REFERENCE_POINT %>) {
			vo.referencePointID = referencePointID;
		}
	...
	function disableLoggingProperties(dataTypeId){
		...
		dijit.byId("referencePointLookup").set('disabled', true);
	...
	function enableLoggingProperties(dataTypeId){
		...
		dijit.byId("referencePointLookup").set('disabled', false);
	...
	function changeLoggingType() {
		...
		*根据类型是否显示引用点
		*/
		if (loggingType == <%=DataPointVO.LoggingTypes.CUSTOM_REFERENCE_POINT%>) {
			show("referencePointSection");
		}else {
			hide("referencePointSection");
		}
	}
  ...
  <table>
    ...
    <tr>
      ...
      <td class="formField"><sst:select id="loggingType"
          ...
          <sst:option
            value="<%=Integer
							.toString(DataPointVO.LoggingTypes.CUSTOM_REFERENCE_POINT)%>">
            <fmt:message key="oge.pointEdit.logging.type.referencePoint" />
          </sst:option>
        </sst:select></td>
    </tr>
    
	<tr id="referencePointSection">
			<td class="formLabelRequired">
				<fmt:message key="oge.pointEdit.logging.referencePoint.point" />
			</td>
			<td class="formField">
				<div id="referencePointLookup"></div>
